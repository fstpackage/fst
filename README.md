...
As you can see from the figure, the benchmark measures higher speeds for the lower compression settings. This is due to the fact that the on-disk filesize of the data frame used is actually significantly smaller than the in-memory size. Package `fst` employs several type-specific byte and bit shifters that are used even in uncompressed mode. For example, a single logical value in R takes up 32 bits in memory, and only 2 bits on-disk (there are 3 possible values: `TRUE`, `FALSE`, and `NA`).

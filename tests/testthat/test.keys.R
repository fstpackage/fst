...
test_that("Fully keyed table", {
  ...
  y <- fstreadproxy("testdata/keys.fst", as.data.table = TRUE)
...
test_that("Missing last key", {
  fstwriteproxy(x, "testdata/keys.fst")
  y <- fstreadproxy("testdata/keys.fst", columns = c("A", "B", "C", "E"), as.data.table = TRUE)
...
test_that("Missing middle key", {
  fstwriteproxy(x, "testdata/keys.fst")
  y <- fstreadproxy("testdata/keys.fst", columns = c("A", "C", "D", "E"), as.data.table = TRUE)
